...
def goodness(base,ntoys,iLabel,options):
    if not options.justPlot:
        ...
        exec_me('combine -M GoodnessOfFit %s  --setParameterRange r=-20,20  --setParameters %s --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r_cat1,r_cat2,r_cat3,r_cat4,r_cat5,r_cat6'% (base,options.setParameters,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
        ...
        if options.algo =='saturated':
             exec_me('combine -M GoodnessOfFit %s  --setParameterRange r=0,7  --setParameters %s -s %i -t %i --toysFrequentist --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r_cat1,r_cat2,r_cat3,r_cat4,r_cat5,r_cat6'% (base,options.setParameters,options.seed,ntoys,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
             #exec_me('combine -M GoodnessOfFit %s  --setParameterRange r=0,7  --setParameters %s -s %i -t %i --toysFrequentist --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r'% (base,options.setParameters,options.seed,ntoys,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
        else:
