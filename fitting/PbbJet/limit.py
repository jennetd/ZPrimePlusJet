...
def goodness(base,ntoys,iLabel,options):
    if not options.justPlot:
        ...
        exec_me('combine -M GoodnessOfFit %s  --setParameterRange r=-20,20 --setParameters %s -s %s -t %i --toysFrequentist --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r'% (base,options.setParameters,optioins.seed,ntoys,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
