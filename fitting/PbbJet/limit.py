...
def goodness(base,ntoys,iLabel,options):
    if not options.justPlot:
        ...
        exec_me('combine -M GoodnessOfFit %s  --setParameterRange r=-20,20 --setParameters %s -s %i -t %i --toysFrequentist --algorithm rs -n %s --freezeParameters %s --redefineSignalPOIs r'% (base,options.setParameters,options.seed,ntoys,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
