...
if __name__ == "__main__":
    ...
    if not options.justPlot: 
        ...
        #exec_me('python buildRhalphabetHbb.py -i %s --ifile-loose %s --scale %f -o %s --nr %i --np %i %s %s --remove-unmatched --prefit --is2017 --addHptShape '%(options.ifile, options.ifile_loose, options.scale, cardsDir1, options.NR1, options.NP1, blindString, pseudoString),options.dryRun )
        #exec_me('python buildRhalphabetHbb.py -i %s --ifile-loose %s --scale %f -o %s --nr %i --np %i %s %s --remove-unmatched --prefit --is2017 --addHptShape '%(options.ifile, options.ifile_loose, options.scale, cardsDir2, options.NR2, options.NP2, blindString, pseudoString),options.dryRun )
        #no loose 
        exec_me('python buildRhalphabetHbb.py -i %s  --scale %f -o %s --nr %i --np %i %s %s --remove-unmatched --prefit --is2017 --addHptShape '%(options.ifile, options.scale, cardsDir1, options.NR1, options.NP1, blindString, pseudoString),options.dryRun )
        exec_me('python buildRhalphabetHbb.py -i %s  --scale %f -o %s --nr %i --np %i %s %s --remove-unmatched --prefit --is2017 --addHptShape '%(options.ifile,  options.scale, cardsDir2, options.NR2, options.NP2, blindString, pseudoString),options.dryRun )
        ...
        #exec_me('python makeCardsHbb.py -i %s --ifile-loose %s -o %s --is2017 --remove-unmatched --no-mcstat-shape'%(options.ifile,options.ifile_loose,cardsDir1),options.dryRun)
        #exec_me('python makeCardsHbb.py -i %s --ifile-loose %s -o %s --is2017 --remove-unmatched --no-mcstat-shape'%(options.ifile,options.ifile_loose,cardsDir2),options.dryRun)
        # no loose file
        exec_me('python makeCardsHbb.py -i %s -o %s --is2017 --remove-unmatched --no-mcstat-shape'%(options.ifile,cardsDir1),options.dryRun)
        exec_me('python makeCardsHbb.py -i %s -o %s --is2017 --remove-unmatched --no-mcstat-shape'%(options.ifile,cardsDir2),options.dryRun)
        ...
        #exec_me('combineCards.py cat1=%s/card_rhalphabet_cat1.txt cat2=%s/card_rhalphabet_cat2.txt cat3=%s/card_rhalphabet_cat3.txt cat4=%s/card_rhalphabet_cat4.txt cat5=%s/card_rhalphabet_cat5.txt cat6=%s/card_rhalphabet_cat6.txt  > %s/card_rhalphabet_r%ip%i.txt'%(cardsDir1,cardsDir1,cardsDir1,cardsDir1,cardsDir1,cardsDir1,cardsDir1,cardsDir1,options.NR1, options.NP1),options.dryRun)
        #exec_me('combineCards.py cat1=%s/card_rhalphabet_cat1.txt cat2=%s/card_rhalphabet_cat2.txt cat3=%s/card_rhalphabet_cat3.txt cat4=%s/card_rhalphabet_cat4.txt cat5=%s/card_rhalphabet_cat5.txt cat6=%s/card_rhalphabet_cat6.txt  > %s/card_rhalphabet_r%ip%i.txt'%(cardsDir2,cardsDir2,cardsDir2,cardsDir2,cardsDir2,cardsDir2,cardsDir2,cardsDir2,options.NR2, options.NP2),options.dryRun)
    ...
    if not options.justPlot:    
        ...
        exec_me('python limit.py -M FTest --datacard %s/card_rhalphabet_muonCR_floatZ_r%ip%i.root --datacard-alt %s/card_rhalphabet_muonCR_floatZ_r%ip%i.root -o %s/ftest_r%ip%i_r%ip%i_muonCR -n %i --p1 %i --p2 %i -t %i --lumi %f %s -r %f --seed %s --freezeNuisances %s'%(cardsDir1, options.NR1, options.NP1, cardsDir2, options.NR2, options.NP2, options.odir, options.NR1, options.NP1, options.NR2, options.NP2, options.n, p1, p2, options.toys, options.lumi, dataString, options.r, options.seed, options.freezeNuisances),options.dryRun)
    else:
